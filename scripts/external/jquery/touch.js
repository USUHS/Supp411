...
(function($) {
    ...
    _isSwipe = function () {
        var move_horizontal, move_vertical, sensitivity = 100, ret;
        ret = false;
        if (CURRENT_TOUCH[0]) {
            move_horizontal = Math.abs(FIRST_TOUCH[0].x - CURRENT_TOUCH[0].x) > sensitivity;
            move_vertical = Math.abs(FIRST_TOUCH[0].y - CURRENT_TOUCH[0].y) > sensitivity;
