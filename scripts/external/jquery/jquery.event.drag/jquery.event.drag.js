...
// Created: 2008-06-04 
// Updated: 2010-06-07
// UPdated: 2012-03-05 - Added touch events Ryan McIlmoyl
...
drag = $special.drag = {
	...
	setup: function(){
		...
		$event.add( this, "mousedown touchstart", drag.init, data );
	...
	teardown: function(){
		...
		$event.remove( this, "mousedown touchstart", drag.init );
	...
	init: function( event ){
		...
		// Only applies to mouse events, skip for touch
		if (event.type === "mousedown" && dd.which > 0 && event.which != dd.which ) 
			return; 
		...
		$event.add( document, "mousemove mouseup touchmove touchend", drag.handler, dd );
	...
	handler: function( event ){ 
		...
		switch ( event.type ){
			...
			case 'touchend':
			case 'mouseup': 
				$event.remove( document, "mousemove mouseup touchmove touchend", drag.handler ); // remove page events
				if ( dd.dragging ){
					if ( dd.drop !== false && $special.drop ) 
