...
!function($){
	...
	$.fn.multiselectable = function(options) {
		if (!options) { options = {} }
		var options = $.extend({}, $.fn.multiselectable.defaults, options);
		
		return this.each(function() {
			var list = $(this)
			if(!list.data("multiselectable-setup")){
				list.data("multiselectable-setup", true);
				var downFn = function(e) {
					var item = $(this),
					parent = item.parent();				
					if (!e.ctrlKey && !e.metaKey && !e.shiftKey && !item.hasClass(options.selectedClass)){						
						parent.find('.' + options.selectedClass).removeClass(options.selectedClass)	
						item.addClass(options.selectedClass);
					}
				};
				list.off("mousedown", options.handle, downFn).on("mousedown", options.handle, downFn);
				...
				list.off("click", options.handle, clickFn).on("click", options.handle, clickFn).disableSelection()
			}
		})
	}
	...
	$.fn.multisortable = function(options) {
		...
		return this.each(function() {
			...
			list.multiselectable({selectedClass: settings.selectedClass, click: settings.click, handle: settings.handle || settings.items})
			...
			options.start = function(event, ui) {
				if (ui.item.hasClass(settings.selectedClass)) {
					...
					parent.find('.' + settings.selectedClass).each(function(i) {
						$(this).addClass(settings.helperClass).data('i', i);
					})
			...
			options.stop = function(event, ui) {
				...
				$("." + settings.helperClass, ui.item.parent()).removeClass(settings.helperClass);
	...
	$.fn.multisortable.defaults = {
		...
		helperClass: 'ui-sortable-helper',
		items: 'li',
		handle: 'li'
	}
