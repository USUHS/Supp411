 ...
 * 
 *
 * &name jsonSuggest (renamed to jsonSearch for the purpose of this copy)
 * &type jQuery plugin
...
(function($) {
	...
	$.fn.jsonSearch = function(settings) {
		var defaults = {  
				url: '',
				data: [],
				minCharacters: 1,
				...
				onSelect: undefined,
				width: undefined,
				//Added for the purpose of this copy to create versatility
				searchKey :"text"
			},
			getJSONTimeout;
		...
		return this.each(function() {
			...
			*/
			function setHoverClass(el) {
				$('li', results).removeClass('resultHover');
				if (el) {
					$(el).addClass('resultHover');
				}
			...
			*/
			function buildResults(resultObjects, filterTxt) {
				...
				for (i = 0; i < resultObjects.length; i += 1) {
					var item = $('<li />'),
						text = resultObjects[i][settings.searchKey];
			...
			function runSuggest(e) {	
				var search = function(searchData) {
					...
					for (i = 0; i < searchData.length; i += 1) {
						if (filterPatt.test(searchData[i][settings.searchKey]) === bMatch) {
